...
module.exports = async (req, res) => {
  ...
  try { initFirebase(); } catch (e) { console.error('[auth/login-firebase] firebase init error', e && e.message); return jsonResponse(res, 500, { error: 'firebase_not_configured' }); }
  ...
  try {
    ...
    const q = await usersRef.where('firebaseUid', '==', firebaseUid).limit(1).get();
    ...
    console.error('[auth/login-firebase] error', e && (e.stack || e.message));
