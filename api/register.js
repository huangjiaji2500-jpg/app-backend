...
module.exports = async (req, res) => {
  ...
  try { initFirebase(); } catch (e) { console.error('[register] firebase init error', e && e.message); return jsonResponse(res, 500, { error: 'firebase_not_configured' }); }
  ...
  if (username.length < 4 || username.length > 64) return jsonResponse(res, 400, { error: 'username_length_invalid' });
  if (password.length < 6) return jsonResponse(res, 400, { error: 'password_too_short' });
