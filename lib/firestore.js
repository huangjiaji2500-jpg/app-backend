...
module.exports.getFirestore = function getFirestore(){
  ...
  // Accept either FIREBASE_SERVICE_ACCOUNT_BASE64 (recommended) or FIREBASE_SERVICE_ACCOUNT
  const saEnvRaw = process.env.FIREBASE_SERVICE_ACCOUNT_BASE64 || process.env.FIREBASE_SERVICE_ACCOUNT || '';
  const sa = parseServiceAccount(saEnvRaw);
  if (!sa) return null;
  try {
