...
async function handlePost(req, res, expectedType){
  ...
  try {
    ...
    try {
      ...
      // amount: prefer approved fields, fallback to requested fields to avoid zero when admin didn't set approved explicitly
      const amount = Number(item.amountApprovedUSDT || item.amountApproved || item.amount || item.amountRequestedUSDT || item.amountRequested || payload.amount || 0) || 0;
